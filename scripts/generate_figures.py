...
def generate_phase_transition_figure():
    ...
    for sigma in sigma_values:
        ...
        network.calendars = np.array([network._random_unit_quaternion() for _ in range(network.n_nodes)])
    ...
    plt.plot(sigma_values, order_parameters, 'bo-', linewidth=2, markersize=6, label='Paramètre d\'ordre R(σ)')
...
def generate_transport_scaling_figure():
    ...
    # Génération de la figure
    plt.figure(figsize=(10, 6))
    plt.loglog(distances, mantice_times, 'ro-', linewidth=2, markersize=6, label='Transport Mantice')
    plt.loglog(distances, diffusion_times, 'bs-', linewidth=2, markersize=6, label='Diffusion classique')
