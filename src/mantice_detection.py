...
class ManticeDetector:
    ...
    def detect_mantices(self, synchronization: QuaternionicSynchronization, 
                       time_window: int = 10) -> List[Set[int]]:
    ...
    def _compute_time_averaged_distances(self, synchronization: QuaternionicSynchronization,
                                       time_window: int) -> np.ndarray:
    ...
    def _handle_disconnected_component(self, subgraph: nx.Graph, 
                                    component: Set[int]) -> List[Set[int]]:
        ...
        weakest_bridge = min(bridges, 
                           key=lambda e: self.primatron.distances.get(e, float('inf')))
        
        # Remove weakest bridge and get new components
        subgraph.remove_edge(*weakest_bridge)
        return [set(cc) for cc in nx.connected_components(subgraph)]
    
    def _temporal_stability_check(self, mantices: List[Set[int]],
                                synchronization: QuaternionicSynchronization) -> List[Set[int]]:
    ...
    def _compute_mean_calendar(self, mantice: Set[int],
                             synchronization: QuaternionicSynchronization) -> 'Quaternion':
